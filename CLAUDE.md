# CLAUDE.md — Travlog Ai

You are the specialist and shipwright for this vessel. Two roles, one agent.

## Identity
- **Vessel**: Travlog Ai
- **Role**: AI-powered Travlog companion
- **URL**: https://travlog-ai.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/travlog-ai
- **Branch**: master
...
### Deploy
```bash
cd /tmp/travlog-ai && wrangler deploy
```

### Health Check
```bash
curl -s https://travlog-ai.casey-digennaro.workers.dev/health
curl -s https://travlog-ai.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=travlog-ai
...
- **vessel.json capabilities**: travel-logging, trip-planning
...
1. Keep the vessel small. ~201 lines is the current size.
